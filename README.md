...
| `fold --session R --actor A --transcript PATH [--format pi\|claude\|rudy] [--cwd] [--finalize] [--summarize-cmd CMD]` | checkpoint compare; below `observeAfterTokens` exit 0; else detach a job that appends observations from the delta, reflects when due, prunes, advances the checkpoint; `--finalize` folds whatever is left and marks it `stable`; format autodetects from the transcript's first line (pi's `"type":"session"`, rudy's `"kind":"session_opened"`/`"fork_point"`, else Claude Code) when omitted |
